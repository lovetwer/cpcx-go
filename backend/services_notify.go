...
func buildWinDigestEmail(ws []WinRecord) string {
	...
	for _, w := range ws {
		...
		money := prizeMoney(w.Lot.Type, tier, w.Draw.PoolAmount)
...
<td style="padding:8px 6px;border-bottom:1px solid #eee;color:#d4380d">%s</td>
</tr>`, TypeName(w.Lot.Type), w.Lot.Issue, w.Lot.RedBalls, w.Lot.BlueBalls, w.Draw.RedBalls, w.Draw.BlueBalls, w.MatchedRed, w.MatchedBlue, tier, money))
	}
	// 奖池信息
	poolInfo := ""
...
<tr style="color:#888;text-align:left"><th style="padding:6px">彩种</th><th style="padding:6px">期号</th><th style="padding:6px">您的号码</th><th style="padding:6px">开奖号码</th><th style="padding:6px">命中</th><th style="padding:6px">几等奖</th><th style="padding:6px">单注奖金</th></tr>
%s
</table>
<p style="margin-top:16px;color:#888;font-size:13px">请凭彩票尽快兑奖。本邮件由大奖来了自动发送。</p>
</div>`, u.Username, len(ws), poolInfo, rows.String())
}
...
// 2026 新规则：双色球增设福运奖（3红+0蓝，仅奖池≥15亿时生效），大乐透合并为7个奖级。
// poolAmount 为该期奖池金额（元），用于判断福运奖是否生效及大乐透奖金升级。
func prizeTier(lotType string, mr, mb int, poolAmount int64) string {
	...
	case "双色球":
		...
		case mb >= 1:
			return "六等奖"
		case mr == 3 && mb == 0 && poolAmount >= 15_0000_0000:
			return "福运奖"
		}
	case "大乐透":
		switch {
...
func bestTierOf(recs []WinRecord) string {
	...
	best, bestR := "", 99
	for _, w := range recs {
		t := prizeTier(w.Lot.Type, w.MatchedRed, w.MatchedBlue, w.Draw.PoolAmount)
		if t == "" {
			continue
		}
...
// handleAdminPreviewMail 发送一封模拟中奖邮件给指定用户，预览中奖通知效果（管理员）
func handleAdminPreviewMail(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		writeError(w, http.StatusNotFound, "找不到用户: "+username)
		return
	}
	if u.Email == "" {
		writeError(w, http.StatusBadRequest, "用户 "+username+" 未配置邮箱")
		return
	}

	// 构造模拟中奖数据
	wins := []WinRecord{
		{User: *u, Lot: Lottery{Type: "ssq", Issue: "2026-08-18", RedBalls: "04,06,14,21,22,33", BlueBalls: "16"},
			Draw: DrawResult{Type: "ssq", Issue: "2026095", RedBalls: "04,06,14,21,22,33", BlueBalls: "16", DrawDate: "2026-08-18", PoolAmount: 628888733},
			MatchedRed: 6, MatchedBlue: 1},
		{User: *u, Lot: Lottery{Type: "ssq", Issue: "2026-08-18", RedBalls: "04,06,14,21,22,33", BlueBalls: "15"},
			Draw: DrawResult{Type: "ssq", Issue: "2026095", RedBalls: "04,06,14,21,22,33", BlueBalls: "16", DrawDate: "2026-08-18", PoolAmount: 628888733},
			MatchedRed: 6, MatchedBlue: 0},
		{User: *u, Lot: Lottery{Type: "dlt", Issue: "26094", RedBalls: "05,14,15,17,33", BlueBalls: "01,07"},
			Draw: DrawResult{Type: "dlt", Issue: "26094", RedBalls: "05,14,15,17,33", BlueBalls: "01,07", DrawDate: "2026-08-19", PoolAmount: 795085879},
			MatchedRed: 5, MatchedBlue: 2},
	}

	html := buildWinDigestEmail(wins)
	subject := buildWinSubject(wins)
	if err := sendMail([]string{u.Email}, subject, html); err != nil {
		writeJSON(w, http.StatusOK, map[string]interface{}{"ok": false, "msg": err.Error()})
		return
	}
	writeJSON(w, http.StatusOK, map[string]interface{}{"ok": true, "msg": "模拟中奖邮件已发送至 " + u.Email})
}
