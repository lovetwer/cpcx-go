...
func pullSSQ() (bool, error) {
	...
	if err := json.Unmarshal(body, &cr); err != nil {
		return false, fmt.Errorf("解析福彩API失败: %w", err)
	}
	if cr.State != 0 {
		return false, fmt.Errorf("福彩API返回 state=%d message=%s", cr.State, cr.Message)
	}
...
func pullDLT() (bool, error) {
	...
	if err := json.Unmarshal(body, &sr); err != nil {
		return false, fmt.Errorf("解析体彩API失败: %w", err)
	}
	if !sr.Success {
		return false, fmt.Errorf("体彩API返回 success=false")
	}
