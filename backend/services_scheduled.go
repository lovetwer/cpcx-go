...
func buildSummary(pendings []pendingLot) ([]summaryRow, error) {
	...
	for _, p := range pendings {
		...
		err := DB.QueryRow(`SELECT red_balls, blue_balls, pool_amount FROM draw_results
			WHERE type=? AND (DATE(draw_date)=? OR issue=?)`, p.Type, p.Issue, p.Issue).Scan(&dr, &db, &poolAmount)
		if err == nil {
			r.Drawn = true
			r.DrawRed = dr
			r.DrawBlue = db
		...
		if status == StatusWin && r.Drawn {
			_, mr, mb := MatchResult(p.Type, p.RedBalls, p.BlueBalls, dr, db, poolAmount)
			r.MatchedRed = mr
			r.MatchedBlue = mb
		}
		// 下单用户
